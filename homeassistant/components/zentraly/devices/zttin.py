...
class ZttinCommands(ZentralyDeviceCommands):
    ...
    LOCK_MODE_ATTRIBUTE_TYPE = DataType.INT16
    ...
    def parse_report_entry(
        self,
        entry: dict[str, Any],
    ) -> tuple[ClimateCapability, Any] | None:
        ...
        if cluster == self.TEMPERATURE_CLUSTER:
            ...
            if attribute_id == self.LOCK_MODE_ATTRIBUTE_ID:
                raw_value = self._parse_integer(value)

                if raw_value not in (0, 1):
                    raise ValueError(f"Invalid ZTTIN lock mode: {raw_value}")
        ...
        if (
            ...
            if raw_value not in (0, 1):
                raise ValueError(f"Invalid ZTTIN heat demand value: {raw_value}")
    ...
    def get_mac_command(self, rid: int) -> dict[str, Any]:
        ...
        return ZentralyCommonCommands.build_read_attr(
            rid=rid,
            mac="",
            cluster=self.MAC_CLUSTER,
            ep=self.ENDPOINT,
            attrs=[
                {
                    "id": self.MAC_ATTRIBUTE_ID,
                    "type": self.MAC_ATTRIBUTE_TYPE,
                }
            ],
        )
    ...
    def build_read_lock_mode(
        ...
        return self._build_read_attribute(
            ...
            mac=mac,
            cluster=self.TEMPERATURE_CLUSTER,
            attribute_id=self.LOCK_MODE_ATTRIBUTE_ID,
            data_type=self.LOCK_MODE_ATTRIBUTE_TYPE,
        )
    ...
    def parse_lock_mode_response(
        ...
        value = self._parse_integer(
            self._parse_attribute_value(
                attrs,
                self.LOCK_MODE_ATTRIBUTE_ID,
            )
        )

        if value not in (0, 1):
            raise ValueError(f"Invalid ZTTIN lock mode: {value}")
    ...
    def build_write_lock_mode(
        ...
        """Build the physical lock write command."""
    ...
    def parse_write_lock_mode_response(
        ...
        """Validate lock mode write response."""
    ...
    def parse_heat_demand_response(
        ...
        if value not in (0, 1):
            raise ValueError(f"Invalid ZTTIN heat demand value: {value}")
