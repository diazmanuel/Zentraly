...
def _create_binary_sensor_entities(
    ...
    entities: list[ZentralyBinarySensor] = []

    if binary_sensor_api.supports(BinarySensorCapability.ON_OFF):
        entities.append(
            ZentralyBinarySensor(
                device=device,
                binary_sensor_api=binary_sensor_api,
                capability=BinarySensorCapability.ON_OFF,
            )
        )

    if binary_sensor_api.supports(BinarySensorCapability.FORCED_MODE):
        entities.append(
            ZentralyBinarySensor(
                device=device,
                binary_sensor_api=binary_sensor_api,
                capability=BinarySensorCapability.FORCED_MODE,
            )
        )
...
class ZentralyBinarySensor(BinarySensorEntity):
    ...
    async def async_update(self) -> None:
        ...
        if self._capability is BinarySensorCapability.ON_OFF:
            value = await self._binary_sensor_api.async_get_on_off()

        elif self._capability is BinarySensorCapability.FORCED_MODE:
            value = await self._binary_sensor_api.async_get_forced_mode()
